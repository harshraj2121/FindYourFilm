...
function Popular() {
    ...
    const getPopular = async () => {
      ...
      useEffect(() => {
        // Reset page and Popular data when category or duration changes
        setPopular([]);
        setPage(1);
    ...
    <div className="pt-[.55%] w-full">
      <nav className="px-[3%] w-full flex items-center justify-center">
        ...
        <div className="flex items-center justify-center gap-2">
          <DropDown title="Popular" opt={["tv", "movie"]} func={(e) => setcategory(e.target.value)} />
        </div>
      </nav>
      <InfiniteScroll
        next={getPopular}  // Load more data on scroll
        hasMore={hasMore}   // Whether there is more data to load
        dataLength={Popular.length}  // Length of current loaded data
